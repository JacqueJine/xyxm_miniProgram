<template>
  <view class="all" v-if="alldata != ''">
    ...
    <view
      class="orderS"
      :style="alldata.order_static == 3 ? 'background: #D4D3D3;' : ''"
    >
      <view>任务状态</view>
      <view style="display: flex; align-items: center; font-size: 40rpx">
        <u-icon
          ...
          v-if="alldata.order_static == 1 || alldata.order_static == 2"
        ></u-icon>
        <u-icon
          name="checkmark-circle"
          size="40rpx"
          style="margin: 2rpx 6rpx 0 0"
          v-if="alldata.order_static == 4"
        ></u-icon>
        <u-icon
          name="info-circle"
          size="40rpx"
          style="margin: 2rpx 6rpx 0 0"
          v-if="alldata.order_static == 3"
        ></u-icon>
        {{
          alldata.order_static == 1 && alldata.getTask.static_cancel.code == 1
            ? "进行中"
            : alldata.order_static == 2
            ? "待结算"
            : alldata.order_static == 3 &&
              alldata.getTask.static_cancel.code == 1
            ? "已取消"
            : (alldata.order_static == 1 &&
                alldata.getTask.static_cancel.code == 0) ||
              (alldata.order_static == 3 &&
                alldata.getTask.static_cancel.code == 0)
            ? "任务取消中"
            : alldata.order_static == 4
            ? "已完成"
            : ""
        }}
      </view>
      <view v-if="alldata.getTask.destination_time.down != ''">
        <u-count-down
          :timestamp="alldata.getTask.destination_time.down"
          bg-color="transparent"
          v-if="
            alldata.order_static == 1 &&
            alldata.getTask.destination_time.down != '已超时' &&
            alldata.getTask.static_cancel.code == 1
          "
        ></u-count-down>
        <text
    ...
    <view
      v-if="
        (alldata.order_static == 1 &&
          alldata.getTask.static_cancel.code == 0) ||
        (alldata.order_static == 3 && alldata.getTask.static_cancel.code == 0)
      "
      style="
        font-size: 24rpx;
        color: #fc463c;
    ...
    <!-- 任务倒计时 -->
    <!-- 		<view v-if="
	    alldata.order_static == 3 && alldata.getTask.static_cancel.code == 1
	  " style="
	    font-size: 24rpx;
	    color: #fc463c;
	    text-align: center;
        ...
        <view class="issuer1">
          <view class="head">
            <image
              class="imgInfo"
              :src="
                alldata.getTask.user_info.header == ''
                  ? '/static/mine/name1.png'
                  : imgBaseUrl + alldata.getTask.user_info.header
              "
            ></image>
            <image
              v-if="alldata.getTask.user_info.show_attestation"
              class="is_auth"
              :src="
                alldata.getTask.user_info.show_attestation == 1
                  ? '/static/index/is_auth.png'
                  : alldata.getTask.user_info.show_attestation == 2
                  ? '/static/index/is_shop.png'
                  : alldata.getTask.user_info.show_attestation == 3
          ...
          <view class="user">
            <view class="user-t user-tt" id="">
              <view class="name"> {{ alldata.getTask.user_name }} </view>
              <image
                class="gender"
                v-if="alldata.getTask.user_info.sex.code != 0"
                :src="
                  alldata.getTask.user_info.sex.code == 1
                    ? '/static/index/man.png'
                    : '/static/index/female.png'
                "
              ></image>
              <!-- <image class="gender" v-if="alldata.getTask.user_info.is_store.code==1" src="/static/mine/shop.png"></image> -->
              <view>
                <text class="xinyu"
                  >信誉{{ alldata.getTask.user_info.credit }}</text
                >
              </view>
            </view>
            <view class="user-t">
              <view class="time">{{ alldata.getTask.create_time }}</view>
              <!-- <view class="location">郑州大学</view> -->
            </view>
          </view>
        </view>
        <u-icon name="more-dot-fill" size="42rpx" @click="moreBtn()"></u-icon>
      </view>

      <view class="per-c">
        <view class="per-c-l">
          <text>{{ alldata.getTask.title }}</text>
          <!-- 加急图片 -->
          <image
            class="imgInfo"
            v-if="
              alldata.getTask.type == 2 && alldata.getTask.is_good_prepay == 1
            "
            src="/static/index/yu.png"
          ></image>
          <image
            class="imgInfo"
            v-if="alldata.getTask.is_urgent == 1"
            src="/static/index/ji.png"
          ></image>
        </view>
        <view class="per-c-r">
          <text>￥{{ alldata.getTask.task_price_total }}</text>
        </view>
      </view>
      <view class="ji">
        <text>跑腿费</text>
        <text>{{ alldata.getTask.money }}元</text>
      </view>
      <view
        class="ji"
        v-if="alldata.getTask.type == 2 && alldata.getTask.is_good_prepay == 1"
      >
        <text>商品预付费</text>
        <text>{{ alldata.getTask.good_prepay }}元</text>
      </view>
      <view class="ji" v-if="alldata.getTask.is_urgent == 1">
        <text
          >加急费加急费（平台收取{{
            alldata.getTask.urgent_price_rate
          }}%手续费）</text
        >
        <text>{{ alldata.getTask.urgent_price }}元</text>
      </view>
      <view class="ji" v-if="alldata.getTask.is_urgent == 1">
        <text>实际到手</text>
    ...
    <view
      ...
      v-if="alldata.getTask.type == 2 && alldata.getTask.is_good_prepay == 1"
    >
      该任务赏金包含 {{ alldata.getTask.good_prepay }}元
      商品预付金额，先由你垫付商品金额完成任务后直接到账余额没有任何手续费，若垫付金额与实际金额有差额请及时与发布者沟通
    </view>
    <!-- 02 快递信息 -->
    <view class="center">
      <view class="c-per">
        <view class="center-per">
          <!-- 快递 -->
          <view v-if="alldata.getTask.type == 1">
            <view
              :style="
                index > 0
              ...
              "
              v-for="(item, index) in alldata.getTask.task_expressage"
              :key="index"
            >
          ...
          <view
            ...
            v-if="alldata.getTask.type == 2"
          >
            <view class="takePlace">
              <view class="dot red"></view>
              <view class="cp-t">取件地点：</view>
              <view class="cp-c">{{ alldata.getTask.task_address }}</view>
            </view>
            <view class="deliveryPlace">
              <view class="dot blue"></view>
              <view class="cp-t">送件地点：</view>
              <view class="cp-c">{{ alldata.getTask.destination }}</view>
            </view>
          </view>
          <!-- 其他任务 -->
          <view v-if="alldata.getTask.type == 3">
            <view class="takePlace">
              <view class="dot red"></view>
              <view class="cp-t">任务内容：</view>
              <view class="cp-c">{{ alldata.getTask.express_desc }}</view>
            </view>
          </view>
          <view class="deliveryPlace" v-if="alldata.getTask.type != 3">
            <view class="dot" style="background: #00c85b"></view>
            <view class="cp-t">送件时间：</view>
            <view class="cp-c"
              >{{ alldata.getTask.destination_time.text }} 前</view
            >
          </view>
          <view class="deliveryPlace" v-if="alldata.getTask.type != 3">
            <view class="cp-t">备注：</view>
            <view class="cp-c notes-c">{{ alldata.getTask.comment }}</view>
          </view>
        </view>
      </view>
    </view>
    <!-- 我已完成 -->
    <view class="compelete" v-if="alldata.order_static != 4">
      <view
        @click="call"
        class="call"
        v-if="
      ...
      <view
        ...
        v-if="
          alldata.order_static == 1 &&
          alldata.getTask.static_cancel.code == 0 &&
          alldata.getTask.static_cancel.cancel_code == 2
        "
      ...
      <view
        ...
        >{{
          alldata.order_static == 1 && alldata.getTask.static_cancel.code == 1
            ...
            : alldata.order_static == 3 &&
              alldata.getTask.static_cancel.code == 0 &&
              alldata.getTask.static_cancel.cancel_code == 1
            ? "撤销“取消订单"
            : ""
        }}</view
      >
...
export default {
  ...
  methods: {
    ...
    getList() {
      listComment({
        where: {
          task_id: this.alldata.getTask.id,
        },
        limit: 10,
        page: this.page,
      }).then((res) => {
    ...
    moreBtn() {
      ...
      // 已取消 已完成可删除
      if (
        (this.alldata.getTask.task_state == 3 &&
          this.alldata.getTask.static_cancel.code == 1) ||
        this.alldata.getTask.task_state == 4
      ) {
        uni.showActionSheet({
          itemList: ["删除订单"],
          itemColor: "#ff0000",
          ...
          success(res) {
            if (res.tapIndex == 1) {
              ...
              uni.showModal({
                ...
                title: "温馨提示",
                content:
                  "您已接单若取消订单将扣除您的信誉分，若重复多次取消将作封号处理",
                success: (res) => {
                  if (res.confirm) {
                    // console.log("用户点击确定");
                    uni.showLoading({
    ...
    call() {
      uni.makePhoneCall({
        phoneNumber: this.alldata.getTask.user_phone,
      });
    },
    submit() {
      if (this.inform == "") {
      ...
      // 任务
        ...
        data = {
          task_id: this.alldata.getTask.id,
          inform_user_id: this.alldata.getTask.user_id,
          inform: this.inform,
        };
        Inform({
          data: data,
    ...
    getOrder() {
      getInfoOrder({
        ...
        if (res.code == 200) {
          ...
          this.getList();
          // this.truemoney = (Number(this.alldata.getTask.true_urgent_price) + Number(this.alldata.getTask.good_prepay) +
          // 	Number(this.alldata.getTask.money)).toFixed(2);
          this.truemoney = this.$Numcheck.rounding(
            Number(this.alldata.getTask.true_urgent_price) +
              Number(this.alldata.getTask.good_prepay) +
              Number(this.alldata.getTask.money),
            2
          );
        }
      });
    ...
    haveCompleted() {
      ...
      // 骑手同意发布人取消任务
        ...
        this.alldata.order_static == 1 &&
        this.alldata.getTask.static_cancel.code == 0
      ) {
        uni.showLoading({
          title: "提交中……",
          mask: true,
