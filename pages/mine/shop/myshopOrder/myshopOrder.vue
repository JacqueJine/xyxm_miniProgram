		...
		<view>
			<view class="top4_2" v-for="(item,index) in list" @click="toDetail(item.id)" :key="index">
				...
				<view>
					...
					<view :class="act==2?'yifu1':act==3 || act==0 || act==1?'yifu2':''">{{act==1?'退款':act==2?'确认收货':act==3?'查看':act==0?'去付款':''}}</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			toDetail(id) {
				uni.navigateTo({
					url: "/pages/mine/shop/myshopOrder/myorderDetail/myorderDetail?id=" + id,
				});
			},
		},
		onReachBottom() {
